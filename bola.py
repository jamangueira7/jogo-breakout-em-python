...
class Bola(Objeto):
    ...
    def update(self, jogo):
        #muda a variavel bola
        #primeiro nos movemos a bola
        self.move(jogo.canvas)

        #depois verificamos por colisoes com as extremidades
        self.chocouBorda(jogo)

        #e por fim vemos se há choques com os retangulos
    ...
    def colideRect(self, rect):
        #detecta se ha colisões ou não com um retangulo
        if (self.x < rect.x + rect.b and self.x + self.r > rect.x and self.y < rect.y + rect.h and self.y + self.r > rect.y):
            #distancia entre o centro do retangulo e o centro da circunferencia
    ...
    def chocouBorda(self, jogo):
        #verifica se a bola chocou-se contra um as bordas
        if self.x < 0:
            self.x = 0
            self.vx = -self.vx
